...
# Import Basi. See https://github.com/Zuzzuc/Basi
...
if [ ! -d "/Applications/Wine.app" ];then
	...
	if [ "$(echo "$Wine" | tr '[:upper:]' '[:lower:]')" == "y" ];then
		BasiPath[0]="remote%http://www.zuzzuc.com/files/Wine.app.tar.gz"
		BasiLoc[0]="$workdir/Wine.app.tar.gz"
		BasiFileAction[0]='echo "Extracting archive and installing application";cat "$BasiActiveFile" | gzip -d | tar -xC "/Applications"'
